 ...
 */
struct ResultSetHeaders(MySQLSocketType socketType)
{
private:
...
public:
     ...
     */
    this(Connection!socketType con, uint fieldCount)
    {
 ...
 */
struct PreparedStmtHeaders(MySQLSocketType socketType)
{
private:
    Connection!socketType _con;
...
public:
    this(Connection!socketType con, ushort cols, ushort params)
    {
        _con = con;
        _colCount = cols;
        _paramCount = params;
 ...
 */
class Connection(MySQLSocketType socketType = MySQLSocketType.vibed) : ConnectionBaseType!socketType
{
protected:
    enum OpenState
    {
 ...
 */
struct Row(MySQLSocketType socketType)
{
private:
    Variant[]   _values;
    bool[]      _nulls;
...
public:
     ...
     * its inherent type information. If this were the case, then the bool array could disappear.
     */
    this(Connection!socketType con, ref ubyte[] packet, ResultSetHeaders!socketType rh, bool binary)
    in
    {
 ...
 */
struct ResultSet(MySQLSocketType socketType)
{
private:
    Row!socketType[] _rows;      // all rows in ResultSet, we store this to be able to revert() to it's original state
    string[]         _colNames;
    Row!socketType[] _curRows;   // current rows in ResultSet

    this (Row!socketType[] rows, string[] colNames)
    {
        _rows = rows;
        _curRows = _rows[];
        _colNames = colNames;
...
public:
     ...
     *
     */
    @property ResultSet!socketType save()
    {
        return this;
    }
     ...
     */
    Row!socketType opIndex(size_t i)
    {
 ...
 */
struct ResultSequence(MySQLSocketType socketType)
{
private:
    Command!(socketType)* _cmd;
    Row!socketType _row; // current row
    string[]       _colNames;
    ulong          _numRowsFetched;
    bool           _empty;

    this (Command!(socketType)* cmd, string[] colNames)
    {
        _cmd        = cmd;
        _colNames   = colNames;
        popFront();
 ...
 */
struct Command(MySQLSocketType socketType)
{
private:
    Connection!socketType _con;
    string _sql;
    ...
    ResultSetHeaders!socketType _rsh;
    PreparedStmtHeaders!socketType _psh;
    Variant[] _inParams;
...
public:
     ...
     */
    this(Connection!socketType con)
    {
        _con = con;
        _con.resetPacket();
    }
     ...
     */
    this(Connection!socketType con, string sql)
    {
        _sql = sql;
        this(con);
    }
    ...
    void prepare()
    {
        ...
        if (packet.front == ResultPacketMarker.ok)
        {
            ...
            _psh = PreparedStmtHeaders!socketType(_con, _fieldCount, _psParams);
        }
     ...
     */
    ResultSet!socketType execSQLResult(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        enforceEx!MYX(execSQL(ra), "The executed query did not produce a result set.");
        _rsh = ResultSetHeaders!socketType(_con, _fieldCount);
        if (csa !is null)
        ...
        while(true)
        {
            ...
            if(packet.isEOFPacket())
                break;
            rows ~= Row!socketType(_con, packet, _rsh, false);
        ...
        return ResultSet!socketType(rows, _rsh.fieldNames);
    }
     ...
     */
    ResultSequence!socketType execSQLSequence(ColumnSpecialization[] csa = null)
    {
        uint alloc = 20;
        Row!(socketType)[] rra;
        rra.length = alloc;
        ...
        _headersPending = false;
        return ResultSequence!socketType(&this, _rsh.fieldNames);
    }
     ...
     * Returns: A (possibly empty) ResultSet.
     */
    ResultSet!socketType execPreparedResult(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        ...
        for (uint i = 0;; i++)
        {
            ...
            if (packet.isEOFPacket())
                break;
            auto row = Row!socketType(_con, packet, _rsh, true);
            if (cr >= alloc)
            {
                alloc = (alloc*3)/2;
                rra.length = alloc;
        ...
        _rowsPending = _pendingBinary = false;
        rra.length = cr;
        auto rs = ResultSet!socketType(rra, _rsh.fieldNames);
        return rs;
    }
     ...
     * Returns: A (possibly empty) ResultSequence.
     */
    ResultSequence!socketType execPreparedSequence(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        enforceEx!MYX(execPrepared(ra), "The executed query did not produce a result set.");
        uint alloc = 20;
    ...
    Row!socketType getNextRow()
    {
        ...
        if (_pendingBinary)
            rr = Row!socketType(_con, packet, _rsh, true);
        else
            rr = Row!socketType(_con, packet, _rsh, false);
        //rr._valid = true;
        return rr;
    }
 ...
 */
struct MetaData(MySQLSocketType socketType)
{
private:
    Connection!socketType _con;
    ...
    MySQLProcedure[] stored(bool procs)
    {
        ...
        auto cmd = Command!socketType(_con, query);
...
public:
    ...
    string[] databases()
    {
        auto cmd = Command!socketType(_con, "SHOW DATABASES");
        auto rs = cmd.execSQLResult();
        string[] dbNames;
    ...
    string[] tables()
    {
        auto cmd = Command!socketType(_con, "SHOW TABLES");
        auto rs = cmd.execSQLResult();
        string[] tblNames;
