 ...
 *    $(LI ResultSequence: $(UL $(LI An input range of similar rows.)))
 * )
 *
 ...
 *
 * To use this module, you MUST include one of the following to choose which type of 
 * socket will be used by default:
 * $(UL
 *   $(LI -version=MySQLN_DefaultSocketVibeD   - Use Vibe.d's sockets by default.)
 *   $(LI -version=MySQLN_DefaultSocketPhobos  - Use Phobos's sockets by default.)
 *   $(LI -version=MySQLN_DefaultSocketNone    - No default, must use the X-suffixed types.)
 * )
 * 
 * No matter which socket type is chosen as default, you can always explicitly choose
 * one in your code by using these X-suffixed types:
 * $(UL
 *   $(LI ResultSetHeadersX!MySQLSocketType)
 *   $(LI PreparedStmtHeadersX!MySQLSocketType)
 *   $(LI ConnectionX!MySQLSocketType)
 *   $(LI RowX!MySQLSocketType)
 *   $(LI ResultSetX!MySQLSocketType)
 *   $(LI ResultSequenceX!MySQLSocketType)
 *   $(LI CommandX!MySQLSocketType)
 *   $(LI MetaDataX!MySQLSocketType)
 * )
 * 
...
version(MySQLN_NoVibeD)
{
    version(MySQLN_DefaultSocketVibeD)
        static assert(false, "Can't use -version=MySQLN_NoVibeD with -version=MySQLN_DefaultSocketVibeD");
}
else
{
...
enum mySQLSocketPhobos = MySQLSocketType.phobos;
enum mySQLSocketVibeD = MySQLSocketType.vibed; 

// Wraps a Phobos Socket with a Vibe.D TcpConnection-like interface
private class PhobosSocketWrapper
{
    private std.socket.TcpSocket socket;
...
version(MySQLN_DefaultSocketVibeD)
{
    alias ResultSetHeadersX   !mySQLSocketVibeD ResultSetHeaders;
    alias PreparedStmtHeadersX!mySQLSocketVibeD PreparedStmtHeaders;
    alias ConnectionX         !mySQLSocketVibeD Connection;
    alias RowX                !mySQLSocketVibeD Row;
    alias ResultSetX          !mySQLSocketVibeD ResultSet;
    alias ResultSequenceX     !mySQLSocketVibeD ResultSequence;
    alias CommandX            !mySQLSocketVibeD Command;
    alias MetaDataX           !mySQLSocketVibeD MetaData;
}
else version(MySQLN_DefaultSocketPhobos)
{
    alias ResultSetHeadersX   !mySQLSocketPhobos ResultSetHeaders;
    alias PreparedStmtHeadersX!mySQLSocketPhobos PreparedStmtHeaders;
    alias ConnectionX         !mySQLSocketPhobos Connection;
    alias RowX                !mySQLSocketPhobos Row;
    alias ResultSetX          !mySQLSocketPhobos ResultSet;
    alias ResultSequenceX     !mySQLSocketPhobos ResultSequence;
    alias CommandX            !mySQLSocketPhobos Command;
    alias MetaDataX           !mySQLSocketPhobos MetaData;
}
else version(MySQLN_DefaultSocketNone)
{}
else
{
    static assert(false, "
mysql-native: You must specify a default type of socket by using one of the following:
    -version=MySQLN_DefaultSocketVibeD   (Use Vibe.d's sockets by default)
    -version=MySQLN_DefaultSocketPhobos  (Use Phobos's sockets by default)
    -version=MySQLN_DefaultSocketNone    (No default)
    ");
}

private template ConnectionBaseType(MySQLSocketType socketType)
{
    version(MySQLN_NoVibeD)
        static assert(socketType != MySQLSocketType.vibed, "Cannot use MySQLSocketType.vibed with -version=MySQLN_NoVibeD");
    
    static if(socketType == MySQLSocketType.phobos)
        alias Object ConnectionBaseType;
    else static if(socketType == MySQLSocketType.vibed)
        alias EventedObject ConnectionBaseType;
    else
        static assert(0);
}

private template SocketOf(MySQLSocketType socketType)
{
    version(MySQLN_NoVibeD)
        static assert(socketType != MySQLSocketType.vibed, "Cannot use MySQLSocketType.vibed with -version=MySQLN_NoVibeD");
    
    static if(socketType == MySQLSocketType.phobos)
        alias PhobosSocketWrapper SocketOf;
    else static if(socketType == MySQLSocketType.vibed)
        alias vibe.core.net.TcpConnection SocketOf;
    else
        static assert(0);
}

private auto connectTcp(MySQLSocketType socketType)(string host, ushort port)
    if(socketType == MySQLSocketType.vibed)
{
    return vibe.core.net.connectTcp(host, port);
}

private auto connectTcp(MySQLSocketType socketType)(string host, ushort port)
    if(socketType == MySQLSocketType.phobos)
{
    auto s = new PhobosSocketWrapper();
    s.connect(host, port);
    return s;
}
 ...
 */
struct ResultSetHeadersX(MySQLSocketType socketType)
{
private:
...
public:
     ...
     */
    this(ConnectionX!socketType con, uint fieldCount)
    {
 ...
 */
struct PreparedStmtHeadersX(MySQLSocketType socketType)
{
private:
    ConnectionX!socketType _con;
...
public:
    this(ConnectionX!socketType con, ushort cols, ushort params)
    {
        _con = con;
        _colCount = cols;
        _paramCount = params;
 ...
 * sequence numbers.
 *
 * For Vibe.d users, 'Sock' should be 'vibe.core.net.TcpConnection'. Otherwise,
 * 'Sock' should be 'std.socket.Socket'.
 */
class ConnectionX(MySQLSocketType socketType) : ConnectionBaseType!socketType
{
protected:
    enum OpenState
    {
    ...
    {
        _socket = connectTcp!socketType(_host, _port);
...
public:
    ...
    static if(socketType == MySQLSocketType.vibed)
    {
 ...
 */
struct RowX(MySQLSocketType socketType)
{
private:
    Variant[]   _values;
    bool[]      _nulls;
...
public:
     ...
     * its inherent type information. If this were the case, then the bool array could disappear.
     */
    this(ConnectionX!socketType con, ref ubyte[] packet, ResultSetHeadersX!socketType rh, bool binary)
    in
    {
 ...
 */
struct ResultSetX(MySQLSocketType socketType)
{
private:
    RowX!socketType[] _rows;      // all rows in ResultSet, we store this to be able to revert() to it's original state
    string[]          _colNames;
    RowX!socketType[] _curRows;   // current rows in ResultSet

    this (RowX!socketType[] rows, string[] colNames)
    {
        _rows = rows;
        _curRows = _rows[];
        _colNames = colNames;
...
public:
     ...
     *
     */
    @property ResultSetX!socketType save()
    {
        return this;
    }
     ...
     */
    @property RowX!socketType front()
    {
     ...
     */
    RowX!socketType opIndex(size_t i)
    {
 ...
 */
struct ResultSequenceX(MySQLSocketType socketType)
{
private:
    CommandX!(socketType)* _cmd;
    RowX!socketType _row; // current row
    string[]        _colNames;
    ulong           _numRowsFetched;
    bool            _empty;

    this (CommandX!(socketType)* cmd, string[] colNames)
    {
        _cmd        = cmd;
        _colNames   = colNames;
        popFront();
 ...
 */
struct CommandX(MySQLSocketType socketType)
{
private:
    ConnectionX!socketType _con;
    string _sql;
    ...
    ResultSetHeadersX!socketType _rsh;
    PreparedStmtHeadersX!socketType _psh;
    Variant[] _inParams;
...
public:
     ...
     */
    this(ConnectionX!socketType con)
    {
        _con = con;
        _con.resetPacket();
    }
     ...
     */
    this(ConnectionX!socketType con, string sql)
    {
        _sql = sql;
        this(con);
    }
    ...
    void prepare()
    {
        ...
        if (packet.front == ResultPacketMarker.ok)
        {
            ...
            _psh = PreparedStmtHeadersX!socketType(_con, _fieldCount, _psParams);
        }
     ...
     */
    ResultSetX!socketType execSQLResult(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        enforceEx!MYX(execSQL(ra), "The executed query did not produce a result set.");
        _rsh = ResultSetHeadersX!socketType(_con, _fieldCount);
        if (csa !is null)
            _rsh.addSpecializations(csa);
        _headersPending = false;

        RowX!(socketType)[] rows;
        while(true)
        {
            auto packet = _con.getPacket();
            if(packet.isEOFPacket())
                break;
            rows ~= RowX!socketType(_con, packet, _rsh, false);
        ...
        return ResultSetX!socketType(rows, _rsh.fieldNames);
    }
     ...
     */
    ResultSequenceX!socketType execSQLSequence(ColumnSpecialization[] csa = null)
    {
        uint alloc = 20;
        RowX!(socketType)[] rra;
        rra.length = alloc;
        ...
        _headersPending = false;
        return ResultSequenceX!socketType(&this, _rsh.fieldNames);
    }
     ...
     * Returns: A (possibly empty) ResultSet.
     */
    ResultSetX!socketType execPreparedResult(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        ...
        for (uint i = 0;; i++)
        {
            ...
            if (packet.isEOFPacket())
                break;
            auto row = RowX!socketType(_con, packet, _rsh, true);
            if (cr >= alloc)
            {
                alloc = (alloc*3)/2;
                rra.length = alloc;
        ...
        _rowsPending = _pendingBinary = false;
        rra.length = cr;
        auto rs = ResultSetX!socketType(rra, _rsh.fieldNames);
        return rs;
    }
     ...
     * Returns: A (possibly empty) ResultSequence.
     */
    ResultSequenceX!socketType execPreparedSequence(ColumnSpecialization[] csa = null)
    {
        ulong ra;
        enforceEx!MYX(execPrepared(ra), "The executed query did not produce a result set.");
        uint alloc = 20;
    ...
    RowX!socketType getNextRow()
    {
        ...
        if (_pendingBinary)
            rr = RowX!socketType(_con, packet, _rsh, true);
        else
            rr = RowX!socketType(_con, packet, _rsh, false);
        //rr._valid = true;
        return rr;
    }
 ...
 */
struct MetaDataX(MySQLSocketType socketType)
{
private:
    ConnectionX!socketType _con;
    ...
    MySQLProcedure[] stored(bool procs)
    {
        ...
        auto cmd = CommandX!socketType(_con, query);
...
public:
    ...
    string[] databases()
    {
        auto cmd = CommandX!socketType(_con, "SHOW DATABASES");
        auto rs = cmd.execSQLResult();
        string[] dbNames;
    ...
    string[] tables()
    {
        auto cmd = CommandX!socketType(_con, "SHOW TABLES");
        auto rs = cmd.execSQLResult();
        string[] tblNames;
